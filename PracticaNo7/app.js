...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function inicializarSistema(usuario) {
        ...
        // Inicializar notas
        let notas = [];
        const notasGuardadas = localStorage.getItem('notasGuardadas');
        if (notasGuardadas) {
            const todasLasNotas = JSON.parse(notasGuardadas);
            notas = todasLasNotas.filter(n => n.creadoPor === usuario.nombre);
        }

        function guardarNotas() {
            const notasExistentes = localStorage.getItem('notasGuardadas');
            let todasLasNotas = notasExistentes ? JSON.parse(notasExistentes) : [];
            
            todasLasNotas = todasLasNotas.filter(n => n.creadoPor !== usuario.nombre);
            todasLasNotas = todasLasNotas.concat(notas);
            
            localStorage.setItem('notasGuardadas', JSON.stringify(todasLasNotas));
        }
        ...
        function mostrarNotas() {
            ...
            for (let i = 0; i < notas.length; i++) {
                ...
                titleInput.addEventListener('blur', function() {
                    notas[i].titulo = titleInput.value;
                    guardarNotas();
                });
                
                const contentTextarea = document.createElement('textarea');
                contentTextarea.className = 'note-content';
                contentTextarea.value = nota.contenido;
                contentTextarea.addEventListener('blur', function() {
                    notas[i].contenido = contentTextarea.value;
                    guardarNotas();
                });
